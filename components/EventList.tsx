    ...
    <div className="px-4 py-4 pb-32">
      ...
      <div className="space-y-8">
        {events.map((evt) => {
          ...
          if (!isPalestra && !isPaid && evt.paymentDueDate) {
            ...
            >
              ...
              <div className="p-3 flex flex-col items-center text-center gap-1">
                 ...
                 <div className="w-full bg-gray-50 dark:bg-white/5 rounded-lg p-2 mb-2 border-y border-gray-100 dark:border-gray-800">
                     <div className={`grid grid-cols-3 gap-1 divide-x divide-gray-200 dark:divide-gray-800 text-[10px] font-bold uppercase tracking-widest ${isPalestra ? 'text-sky-500/50' : 'text-gray-400'}`}>
                          ...
                          <div className="pt-1.5 border-t border-gray-100 dark:border-gray-800 space-y-1">
                            {payments.map((p, idx) => (
                              <div key={p.id || idx} className="flex justify-between items-center text-[10px]">
                                ...
                                <span className={`font-bold ${isPalestra ? 'text-sky-500' : 'text-emerald-600'}`}>R$ {parseCurrency(p.amount).toFixed(2).replace('.', ',')}</span>
                              </div>
                            ))}
                            {payments.length > 1 && (
                                <div className={`flex justify-between items-center text-[10px] pt-1 font-black border-t ${isPalestra ? 'text-sky-500 border-sky-50' : 'text-emerald-600 border-emerald-50'}`}>
                 ...
                 {isPalestra ? (
                    !isPaid && (
                        <div className="w-full bg-sky-50 dark:bg-sky-900/20 rounded-lg p-2.5 mb-2 flex items-center justify-center border border-sky-100 dark:border-sky-800 overflow-hidden">
                            <span className="text-sky-500 dark:text-sky-400 text-[12px] font-black uppercase tracking-tighter text-center">
