...
import { TrendingDownIcon, PlusIcon, TrashIcon, CalendarIcon, BoxIcon, DollarSignIcon, XIcon, ChevronLeftIcon } from './Icons';
...
export const ExpensesScreen: React.FC<ExpensesScreenProps> = ({ expenses, events, courseTypes, lectureModels, onAddExpense, onDeleteExpense, onClose }) => {
  ...
  const filteredExpenses = useMemo(() => {
    ...
    events.forEach(evt => {
        ...
        if (matchType && d.getFullYear() === selectedYear && (selectedMonth === 'all' || d.getMonth() === selectedMonth) && (selectedDay === 'all' || d.getDate() === Number(selectedDay))) {
            const materials = isPal ? (evt.materials || []) : (evt.materials || []).filter(m => m.checked);
            materials.forEach(m => {
                automatic.push({
                    id: m.id,
  ...
  const { totalRevenue, totalExpenses, netBalance } = useMemo(() => {
    ...
    if (filterType === 'palestras') {
        events.forEach(evt => {
            if (!evt.date) return;
            if (evt.includeInAnnualRevenue === false) return;
            const d = new Date(evt.date);
            const isPal = checkIfPalestra(evt);
            ...
            if (isPal && matchDate) {
                const baseVal = parseCurrency(evt.value) || 0;
                rev += (evt.palestraType === 'MEU') ? baseVal * (evt.studentCount || 1) : baseVal;
            }
        });
    }
        ...
        <div className="flex flex-col mb-6 gap-4">
            <div className="flex items-center justify-between w-full">
                ...
                {onClose && (
                  <button 
                    onClick={onClose}
                    className="p-2.5 rounded-2xl bg-gray-100 dark:bg-white/5 text-gray-500 hover:text-primary hover:bg-gray-200 dark:hover:bg-white/10 transition-all active:scale-95 border border-gray-200 dark:border-gray-800 shadow-sm"
                    title="Voltar"
                  >
        ...
        <div className="flex gap-2 mb-6">
                ...
                <div className="flex-1 relative">
                    <select 
                        ...
                        className={`w-full appearance-none bg-white dark:bg-surface-dark py-3 pl-3 pr-8 rounded-xl text-[11px] font-black uppercase text-gray-700 dark:text-white focus:outline-none focus:ring-2 ${focusRing} shadow-sm cursor-pointer border border-gray-100 dark:border-gray-700`}
                    >
                ...
                <div className="w-20 relative">
                    <select 
                        ...
                        className={`w-full appearance-none bg-white dark:bg-surface-dark py-3 pl-2 pr-2 rounded-xl text-[11px] font-black uppercase text-gray-700 dark:text-white focus:outline-none focus:ring-2 ${focusRing} shadow-sm cursor-pointer border border-gray-100 dark:border-gray-700 text-center`}
                    >
        ...
        <div className="space-y-3">
            {filteredExpenses.length === 0 ? (
                ...
                filteredExpenses.map(expense => (
                    <div key={expense.id} className={`bg-white dark:bg-surface-dark p-4 rounded-xl shadow-sm border flex justify-between items-center transition-all ${expense.type === 'automatic' ? 'border-primary/10 dark:border-primary/20' : 'border-gray-100 dark:border-gray-800'}`}>
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${expense.type === 'automatic' ? 'bg-primary/5 text-primary dark:bg-primary/10' : 'bg-gray-100 dark:bg-white/10 text-gray-500'}`}>
