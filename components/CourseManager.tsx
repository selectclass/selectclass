...
import { PlusIcon, TrashIcon, PencilIcon, XIcon, CheckIcon, GraduationCapIcon, MenuIcon, ClockIcon, DollarSignIcon, CalendarIcon, MapPinIcon, HomeIcon, ChevronLeftIcon } from './Icons';
...
export const CourseManager: React.FC<CourseManagerProps> = ({ courseTypes, onAddCourse, onUpdateCourse, onRemoveCourse, onSaveOrder, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const courseData: CourseType = {
        ...
        name: name.trim(),
        model: 'Curso',
        defaultValue: parseCurrency(value),
        defaultTime: time,
  ...
  const handleEditClick = (course: CourseType) => {
      ...
      setName(course.name);
      setValue(course.defaultValue !== undefined ? formatCurrencyInput(course.defaultValue) : '');
  ...
  return (
    <div className="p-6">
      ...
      <div className={`rounded-2xl shadow-sm p-6 mb-6 transition-colors duration-300 ${editingId ? 'bg-primary/5 dark:bg-primary/10 border border-primary/20 dark:border-primary/40' : 'bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800'}`}>
        <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-primary/5 dark:bg-primary/20 flex items-center justify-center text-primary">
          ...
          <ul className="divide-y divide-gray-100 dark:divide-gray-800">
            {localList.map((course, index) => (
              <li 
                ...
                className={`p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-move group ${editingId === course.id ? 'bg-primary/5 dark:bg-primary/10' : ''}`}
              >
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                    <button onClick={() => handleEditClick(course)} className="p-2 text-gray-400 hover:text-primary transition-colors"><PencilIcon className="w-4 h-4" /></button>
