...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin, loginError }) => {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-6 bg-primary">
      <div className="w-full max-sm flex flex-col items-center animate-fade-in">
        ...
        <div className="mb-10 text-center">
            ...
            <p className="text-white/60 text-[9px] mt-2 font-bold tracking-widest uppercase opacity-90">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
            <div className="relative">
                ...
                <input 
                    ...
                    className="w-full pl-12 pr-5 py-4 rounded-xl bg-white text-gray-900 placeholder-gray-400 font-medium focus:outline-none focus:ring-4 focus:ring-primary/30 transition-all shadow-lg"
                />
            </div>
            <div className="relative">
                <div className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400">
            ...
            <div className="flex items-center gap-3 pl-1">
                <div className="relative flex items-center">
                    <input 
                        ...
                        className="peer h-5 w-5 cursor-pointer appearance-none rounded border border-white/20 bg-white/10 checked:bg-white checked:border-white transition-all"
                    />
                    <svg className="absolute w-3.5 h-3.5 text-primary pointer-events-none opacity-0 peer-checked:opacity-100 left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-opacity" viewBox="0 0 14 14" fill="none">
                        <path d="M3 8L6 11L11 3.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                </div>
                <label htmlFor="saveAccess" className="text-white/80 text-xs font-bold uppercase tracking-wider cursor-pointer select-none">
                    SALVAR ACESSO
                </label>
            </div>
            ...
            >
                <LockIcon className="w-5 h-5 text-white/60 group-hover:text-white transition-colors" /> 
                ENTRAR
            </button>
        </form>
        
        {/* PWA Install Button - Hidden if already standalone */}
        {!isStandalone && (
          <button
              onClick={handleInstallClick}
              className="mt-6 w-full py-3 bg-white text-primary rounded-xl font-bold flex items-center justify-center gap-3 shadow-lg active:scale-95 transition-all animate-bounce-short border border-transparent hover:border-white/20"
          >
        ...
        <p className="mt-8 text-white/40 text-xs">Powered by JG Creator</p>
