...
import { SearchIcon, CalendarIcon, CheckCircleIcon, AlertCircleIcon, BoxIcon, XIcon, ChevronLeftIcon } from './Icons';
...
export const HistoryScreen: React.FC<HistoryScreenProps> = ({ events, courseTypes, onClose }) => {
  const now = new Date();
        ...
        <div className="flex items-center justify-between mb-6">
            <div>
                <div className="flex items-center gap-2">
                    ...
                    {onClose && (
                      <button 
                        onClick={onClose}
                        className="p-1.5 rounded-full bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-primary transition-all active:scale-95 sm:hidden"
                      >
            ...
            <div className="flex items-center gap-2">
                ...
                {onClose && (
                  <button 
                    onClick={onClose}
                    className="hidden sm:flex p-2.5 rounded-2xl bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-primary hover:bg-gray-200 dark:hover:bg-white/10 transition-all active:scale-95 border border-gray-200 dark:border-gray-800 shadow-sm"
                    title="Voltar"
                  >
        ...
        <div className="space-y-3">
            ...
            {filteredEvents.length === 0 ? (
                ...
                filteredEvents.map((event, index) => {
                    ...
                    const totalValue = (isPal && event.palestraType === 'MEU') ? baseValue * (event.studentCount || 1) : baseValue;
                    const isFullyPaid = (event.paymentStatus === 'paid') || (totalPaid >= (totalValue - 0.01));
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-4 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight">
                                    ...
                                    <span className={isFullyPaid ? 'text-emerald-500' : 'text-red-500'}>
                                        {isFullyPaid ? '✓ Finalizado' : '! Pendente'}
                                    </span>
                                </div>
                                {event.createdAt && (
                                  <p className="text-[8px] text-gray-400 dark:text-gray-500 font-medium mt-2 pt-2 border-t border-gray-50 dark:border-gray-800">
