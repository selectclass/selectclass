...
import { BarChartIcon, MapPinIcon, XIcon, ChevronLeftIcon } from './Icons';
...
export const AnalyticsScreen: React.FC<AnalyticsScreenProps> = ({ events, courseTypes, lectureModels, onClose }) => {
  const now = new Date();
        ...
        <div className="flex flex-col mb-6 gap-4">
            <div className="flex items-center justify-between w-full">
                <div className="min-w-0">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-1">Ranking de {filterType === 'palestras' ? 'Palestras' : 'Cursos'}</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Análise e desempenho.</p>
                </div>
                {onClose && (
                  <button 
                    onClick={onClose}
                    className="p-2.5 rounded-2xl bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-primary hover:bg-gray-200 dark:hover:bg-white/10 transition-all active:scale-95 border border-gray-200 dark:border-gray-800 shadow-sm"
                    title="Voltar"
                  >
            ...
            <div className="flex bg-gray-100 dark:bg-white/5 p-1 rounded-full border border-gray-200 dark:border-gray-800 shadow-inner self-start">
                <button 
                    ...
                    className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${filterType === 'cursos' ? 'bg-primary text-white shadow-md' : 'text-gray-400 hover:text-gray-600'}`}
                >
                    Cursos
                </button>
                <button 
                    onClick={() => { setFilterType('palestras'); setSelectedCity('all'); }}
                    className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase transition-all ${filterType === 'palestras' ? 'bg-primary text-white shadow-md' : 'text-gray-400 hover:text-gray-600'}`}
                >
                    Palestras
                </button>
            </div>
        ...
        <div className="flex gap-2 mb-6">
             <div className="flex-[1.5] relative">
                <select 
                    ...
                    className="w-full pl-3 pr-8 py-3 rounded-xl bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800 text-[11px] sm:text-sm font-bold text-gray-700 dark:text-white focus:ring-2 focus:ring-primary outline-none appearance-none shadow-sm transition-all truncate"
                >
             ...
             <div className="flex-1 relative">
                <select 
                    ...
                    className="w-full pl-3 pr-8 py-3 rounded-xl bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800 text-[11px] sm:text-sm font-bold text-gray-700 dark:text-white focus:ring-2 focus:ring-primary outline-none appearance-none shadow-sm transition-all"
                >
             ...
             <div className="w-20 relative">
                <select 
                    ...
                    className="w-full px-1 py-3 rounded-xl bg-white dark:bg-surface-dark border border-gray-100 dark:border-gray-800 text-[11px] sm:text-sm font-bold text-gray-700 dark:text-white focus:ring-2 focus:ring-primary outline-none appearance-none shadow-sm transition-all text-center"
                >
                            ...
                            >
                                ...
                                <div 
                                    className={`w-3 sm:w-4 rounded-full transition-all duration-500 ease-out relative z-10 shadow-sm
                                        ${isSelected 
                                            ? filterType === 'palestras' ? 'bg-gradient-to-t from-sky-500 to-sky-300 shadow-sky-500/30' : 'bg-gradient-to-t from-primary to-primary shadow-primary/30' 
                                            : 'bg-gray-300 dark:bg-gray-700 hover:bg-primary/30 dark:hover:bg-gray-600'
                                        }
                                    `}
