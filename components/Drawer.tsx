...
export const Drawer: React.FC<DrawerProps> = ({ isOpen, onClose, currentView, onChangeView, onLogout }) => {
  const menuItems = [
    ...
    { view: AppView.ADD_EVENTS, label: 'Adicionar Curso', icon: <GraduationCapIcon className="w-5 h-5" /> },
  ...
  return (
    <>
      ...
      />
      <div 
        className={`fixed top-0 left-0 h-full w-72 bg-white dark:bg-surface-dark shadow-2xl z-[70] transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex flex-col h-full">
          <div className="h-32 bg-primary flex flex-col justify-end p-5">
             ...
             <p className="text-white/60 text-[8px] font-normal uppercase tracking-widest opacity-90">Excelência em cada agendamento</p>
          </div>
                  ...
                  <button
                    ...
                    className={`w-full flex items-center px-6 py-2.5 text-left transition-colors text-[17px]
                      ${currentView === item.view 
                        ? 'bg-primary/10 text-primary dark:text-primary/70 font-normal border-r-4 border-primary' 
