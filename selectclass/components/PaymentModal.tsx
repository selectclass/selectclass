...
interface PaymentModalProps {
  ...
  onConfirmPayment: (amount: number, date: Date) => void;
}
        ...
        <div className="bg-white dark:bg-surface-dark w-full max-w-sm rounded-2xl shadow-2xl pointer-events-auto transform transition-all scale-100 p-6 relative">
          ...
          <div className="mb-6 bg-gray-50 dark:bg-white/5 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
             <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-semibold dark:text-gray-300">R$ {totalValue.toFixed(2)}</span>
             </div>
             <div className="flex justify-between text-sm mb-1">
             ...
             <div className="flex justify-between text-sm pt-2 border-t border-gray-200 dark:border-gray-700 mt-2">
                ...
                <span className="font-bold text-red-500">R$ {remaining.toFixed(2)}</span>
             </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 mt-6">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-3 px-4 rounded-xl text-gray-500 font-medium hover:bg-gray-100 dark:hover:bg-white/5"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 px-4 rounded-xl bg-green-600 text-white font-bold shadow-lg hover:bg-green-700 transition-all"
                >
                  Confirmar
                </button>
            </div>
