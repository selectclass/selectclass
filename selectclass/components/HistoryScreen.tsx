...
export const HistoryScreen: React.FC<HistoryScreenProps> = ({ events }) => {
  const now = new Date();
    ...
    <div className="p-4 pb-32">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            placeholder="Buscar aluna ou curso neste mês..."
            value={searchTerm}
                    ...
                    return (
                        <div key={event.id} className="bg-white dark:bg-surface-dark p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start gap-4">
                            ...
                            <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center
                                ${isFullyPaid ? 'bg-green-100 text-green-600 dark:bg-green-900/20' : 'bg-red-100 text-red-500 dark:bg-red-900/20'}
                            `}>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start">
                                    <h4 className="font-bold text-gray-800 dark:text-white text-base truncate pr-2">{event.student}</h4>
                                ...
                                <p className="text-sm text-primary dark:text-blue-300 font-medium truncate mb-1">{event.title}</p>
                                
                                <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                                    <span className="flex items-center gap-1">
                                        <BoxIcon className="w-3 h-3" /> {event.materials?.length || 0} materiais
                                    </span>
                                    <span>
